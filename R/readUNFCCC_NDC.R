...
readUNFCCC_NDC <- function(subtype) {
  ...
  if (grepl("Emissions", subtype, fixed = TRUE)) {
    input <- readxl::read_excel(
      ...
      skip = 4,
      na = c("?", ""),
      col_names = c("ISO_Code", "Reference_Year", "BAU_or_Reference_emissions_in_MtCO2e", "Target_Year", "Type",
                    "Unconditional", "Conditional", "Uncond2", "Cond2"),
      col_types = c("skip", "text", "skip", "skip", "skip", "skip", "text", "numeric", "numeric", "text", "numeric",
    ...
    # Replace NAs and drop extra columns
    ...
    input <- input %>%
      dplyr::mutate(Reference_Year = dplyr::case_match(.data$Reference_Year,
                                                       "BAU" ~ -1,
                                                       "no" ~ -2,
