...
readLee <- function(subtype) {
  ...
  avi <- reshape2::melt(avi, id = c("Year", "Tg")) # from wide to long format, keep only Year and Tg
  avi <- data.table::dcast(avi, Year + variable ~ Tg) # from long to wide format, bring contents of Tg to columns and differentiate by Year and variable
