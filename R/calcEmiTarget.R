...
#' @param sources A string: "UNFCCC_NDC"
...
#'
calcEmiTarget <- function(sources = "UNFCCC_NDC", subtype, scenario) {
  ...
  # Get gdp and pop scenarios (make sure to always get SSP2), and filter ghgfactor for those scenarios
  gdp <- calcOutput("GDP", scenario = c("SSP2", scenario), aggregate = FALSE)
  ghgfactor <- mselect(ghgfactor, "data" %in% getNames(gdp))
